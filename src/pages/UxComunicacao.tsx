...
import { Palette, Clock, AlertCircle, ClipboardList, UserCheck } from "lucide-react";
...
export default function UxComunicacao() {
  const { role } = useAuth();
  const { strategies, loading, assignUxCollaborator } = useDbStrategies();
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="mb-2">
          ...
          <p className="text-muted-foreground">Distribuição de estratégias urgentes para colaboradores UX.</p>
        </div>
            ...
            {distributed.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="font-heading font-semibold text-xl text-foreground">
                    Já Distribuídas
                    <span className="ml-2 text-sm font-normal text-muted-foreground">({distributed.length})</span>
                  </h2>
                </div>
                {distributed.map((s) => {
                  ...
                  return (
                    <Card key={s.id} className="p-4 border-success/20 bg-success/5">
            ...
            return (
              <Card key={s.id} className="p-5">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <p className={`text-xs flex items-center gap-1 shrink-0 ml-3 ${isOverdue ? "text-destructive" : "text-muted-foreground"}`}>
